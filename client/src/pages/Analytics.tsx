...
const PERSONA_META: Record<string, { label: string; icon: string; color: string }> = {
  loyal:           { label: "Loyal",           icon: "🛡️", color: "text-blue-400" },
  price_sensitive: { label: "Price-Sensitive", icon: "💰", color: "text-green-400" },
  tech_savvy:      { label: "Tech-Savvy",      icon: "🔬", color: "text-violet-400" },
  risk_averse:     { label: "Risk-Averse",     icon: "🔒", color: "text-amber-400" },
  trend_seeking:   { label: "Trend-Seeking",   icon: "🚀", color: "text-pink-400" },
};

const SCENARIO_META: Record<string, { label: string; short: string; color: string }> = {
  S1: { label: "Personalized",   short: "S1", color: "text-violet-400" },
  S2: { label: "Generic",        short: "S2", color: "text-slate-400" },
  S3: { label: "Evidence",       short: "S3", color: "text-blue-400" },
  S4: { label: "Vague",          short: "S4", color: "text-orange-400" },
  S5: { label: "High Pressure",  short: "S5", color: "text-red-400" },
  S6: { label: "Low Pressure",   short: "S6", color: "text-green-400" },
  S7: { label: "Price-Value",    short: "S7", color: "text-amber-400" },
  S8: { label: "Innovation",     short: "S8", color: "text-cyan-400" },
};

function PersonaBreakdownSection() {
  const { data, isLoading } = trpc.analytics.personaBreakdown.useQuery();

  if (isLoading) return (
    <div className="mb-8">
      <h2 className="text-2xl font-bold mb-1">Persona Breakdown</h2>
      <p className="text-sm text-muted-foreground mb-6">Conversion rates and response metrics per consumer persona</p>
      <div className="p-8 rounded-xl border border-border/50 bg-card text-center">
        <Loader2 className="w-6 h-6 animate-spin mx-auto text-accent" /><p className="text-muted-foreground mt-2">Loading...</p>
      </div>
    </div>
  );

  const hasData = data && data.some(d => d.totalSessions > 0);
  ...
  return (
    <div className="mb-8">
      ...
      {!hasData ? (
        <div className="p-8 rounded-xl border border-border/50 bg-card text-center">
          <p className="text-muted-foreground">No persona sessions yet. Run experiments with personas to see breakdown.</p>
        </div>
      ) : (
        <div className="rounded-xl border border-border/50 bg-card overflow-hidden">
          <div className="grid grid-cols-6 text-xs text-muted-foreground font-medium px-5 py-3 border-b border-border/30 bg-secondary/30">
            <div className="col-span-2">Persona</div>
            <div className="text-right">Sessions</div>
            <div className="text-right">Conv. Rate</div>
            <div className="text-right">Avg Sentiment</div>
          ...
          <div className="divide-y divide-border/20">
            {data!.map(row => {
              ...
              return (
                <div key={row.persona} className="grid grid-cols-6 items-center px-5 py-3.5 hover:bg-secondary/20 transition-colors">
                  <div className="col-span-2 flex items-center gap-2">
                    <span className="text-lg">{meta.icon}</span>
                    <div>
                      <div className={`text-sm font-semibold ${meta.color}`}>{meta.label}</div>
                      <div className="text-[10px] text-muted-foreground">{row.avgResponseTime > 0 ? `${row.avgResponseTime}ms avg` : "—"} · {row.avgResponseLength > 0 ? `${row.avgResponseLength} chars` : "—"}</div>
                    </div>
                  </div>
                  <div className="text-right text-sm font-medium">{row.totalSessions}</div>
                  <div className="text-right">
                    <span className={`text-sm font-bold ${row.conversionRate >= 50 ? "text-emerald-400" : row.conversionRate >= 25 ? "text-amber-400" : "text-red-400"}`}>
                      {row.conversionRate}%
                    </span>
                  </div>
                  <div className="text-right">
                    <span className={`text-sm font-medium ${row.avgSentiment > 20 ? "text-emerald-400" : row.avgSentiment > 0 ? "text-amber-400" : "text-muted-foreground"}`}>
                      {row.avgSentiment !== 0 ? row.avgSentiment : "—"}
                    </span>
                  </div>
                  <div className="text-right">
                    <span className="text-sm text-muted-foreground">
                      {row.avgSatisfaction > 0 ? `${row.avgSatisfaction}/5` : "—"}
                    </span>
                  </div>
                </div>
              );
...
function ScenarioBreakdownSection() {
  const { data, isLoading } = trpc.analytics.scenarioBreakdown.useQuery();

  if (isLoading) return (
    <div className="mb-8">
      <h2 className="text-2xl font-bold mb-1">Scenario Effectiveness</h2>
      <p className="text-sm text-muted-foreground mb-6">Which manipulation condition drives highest brand-switching intent?</p>
      <div className="p-8 rounded-xl border border-border/50 bg-card text-center">
        <Loader2 className="w-6 h-6 animate-spin mx-auto text-accent" /><p className="text-muted-foreground mt-2">Loading...</p>
      </div>
    </div>
  ...
  const hasData = data && data.some(d => d.totalSessions > 0);
  const maxRate = hasData ? Math.max(...data!.map(d => d.conversionRate)) : 100;
  ...
  return (
    <div className="mb-8">
      ...
      {!hasData ? (
        <div className="p-8 rounded-xl border border-border/50 bg-card text-center">
          <p className="text-muted-foreground">No scenario data yet. Complete sessions to compare scenario effectiveness.</p>
        </div>
      ) : (
              ...
              return (
                <div key={row.scenario}>
                  <div className="flex items-center justify-between text-sm mb-1.5">
                    <div className="flex items-center gap-2">
                      <span className={`font-bold ${meta.color}`}>{meta.short}</span>
                      <span className="text-muted-foreground">{meta.label}</span>
                      <span className="text-[10px] text-muted-foreground/50">{row.totalSessions} sessions</span>
                    </div>
                  ...
                  <div className="h-2 rounded-full bg-secondary overflow-hidden">
                    <div
                      ...
                      style={{ width: `${maxRate > 0 ? (row.conversionRate / maxRate) * 100 : 0}%` }}
                    />
                  </div>
                </div>
              );
